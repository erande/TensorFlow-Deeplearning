...
# [0, 11, 0, 10, 9, 0, 12, 0], 第4个位置为9，第3个位置为10，第1个位置为11，第7个位置为12
...
y = tf.linspace(-2., 2, 5)
x = tf.linspace(-2., 2, 5)
points_x, points_y = tf.meshgrid(x, y)  # shape=([5, 5])
# points_x: [-2, -1, 0, 1, 2] 重复5行
# points_y：[-2, -1, 0, 1, 2].T 重复5列
points = tf.stack([points_x, points_y], axis=2)  # shape=([5, 5, 2]), 5个5x2的矩阵
print(points)
